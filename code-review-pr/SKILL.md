---
name: code-review-pr
description: '이 저장소에 올라온 PR 을 빌트인 code-review 로 리뷰하되, CLAUDE.md 의 프로젝트 규칙(테스트 정책, okf/ 동기화, MyBatis 규약, Spring Boot 4 이름, 보안 정책)을 명시적 체크 축으로 얹어 팀원 전원이 같은 기준으로 보게 한다. "PR 리뷰해줘", "PR 번호로 리뷰", "#123 리뷰", "이 PR 문제 없는지 봐줘", "코드리뷰 돌려줘" 처럼 PR 검토를 요청하면 반드시 이 스킬을 쓴다. 머지 직전 최종 관문(체크아웃 + 적대적 리뷰 + PR 코멘트 등록)은 담당하지 않는다 — 그건 code-review-final 이다.'
---
