...
리뷰 사다리와 별개인 인터뷰 스킬이다. 계획·결정·아이디어를 두고 질문을 **한 번에
하나씩** 던지며 결정 트리의 가지를 끝까지 따라간다. 질문마다 추천 답을 붙이고, 파일
시스템이나 도구로 확인할 수 있는 사실은 묻지 않고 직접 찾는다. 결정은 사용자에게
맡기고, 서로 이해가 맞았다고 사용자가 확인하기 전에는 실행하지 않는다.
"grill" 이 들어간 요청에 뜬다.

`mattpocock/skills` 의 `grilling` 을 2026-07-13 커밋 `170ad48` 시점 그대로 가져온
것이다. 이후 원본은 여러 질문을 라운드로 묶어 묻는 방식으로 바뀌었지만, 한 번에
하나씩 묻는 이 버전을 일부러 고정했다. 원본이 갱신돼도 자동으로 따라오지 않는다.
