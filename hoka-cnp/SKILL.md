---
name: hoka-cnp
description: '변경을 의도 단위로 분할 커밋하고 원격에 푸시한다. 커밋 메세지는 `<type>:[<태그>] <한글 subject>` 형식이고, 태그는 커밋에 담긴 파일의 최상위 디렉터리로 정한다 — `brand`, `brand-fo`, `brand-bo` 중 하나, 셋 다 아니면 `common`. "커밋하고 푸시해줘", "커밋 후 푸시", "commit and push", "커밋해줘", "푸시해줘" 처럼 커밋/푸시를 요청할 때 반드시 이 스킬을 쓴다. 커밋 메세지만 작성하거나 메세지 규칙을 물어볼 때도 이 규칙을 따른다.'
---
...
태그는 저장소 이름이 아니라 **그 커밋이 어느 프로젝트를 바꾸는지**다. `brand`,
`brand-fo`, `brand-bo` 가 한 저장소 루트 아래 디렉터리로 같이 있어서 저장소명으로는
구분이 안 되고, 커밋 로그를 프로젝트별로 걸러 보는 것이 이 태그를 다는 유일한 이유다.
...
| `brand-fo` | `brand-fo` |
| `brand-bo` | `brand-bo` |
| 그 밖 (`okf/`, `.claude/`, 루트 파일 등) | 루트 파일 |

그 다음 커밋 단위로 정한다:

- **프로젝트 파일이 한 프로젝트 것뿐이면 그 프로젝트.** 같이 들어간 루트 파일은 따라간다 —
  `brand/…/SampleService.java` + `okf/api/samples.md` 는 `[brand]`. 코드와 그 문서는 한
  커밋에 있어야 하고, 문서가 끼었다고 태그가 바뀌면 `[brand]` 로 걸렀을 때 그 변경이 빠진다.
- **루트 파일뿐이면 `common`.** 내용이 특정 프로젝트 얘기여도(`okf/api/samples.md` 단독)
  `common` 이다. 내용으로 판단하면 세션마다 값이 갈리고, 같은 종류의 커밋이 두 태그로 흩어진다.
- **두 프로젝트 이상이 섞이면 태그를 고르지 않는다.** 커밋을 나눈다 (아래 "커밋 분할").

주의:
- 첫 세그먼트가 **정확히 같은지** 본다. `brand` 로 시작하는지를 보면 `brand-fo/` 가 `brand` 로 잡힌다.
...
- 커밋 직전에 스테이징된 파일로 한 번 더 확인한다. 계획과 다르면 이미 스테이징돼 있던 파일이 섞인 것이다:
  ...
  ```
  `--no-renames` 는 이동의 옛 경로도 보이게 한다. 빼면 `brand/` 밖으로 옮긴 커밋이 `common` 으로 잡힌다.
...
- **프로젝트가 다르면 나눈다.** 태그는 커밋에 하나뿐이라, `brand/` 와 `brand-fo/` 를 한 커밋에 넣으면
  어느 태그로 걸러도 절반이 빠진다. 한 작업(API 필드 추가 + 화면 반영)이어도 프로젝트별로 나누고
  의존 순서대로 커밋한다. 루트 파일은 그 파일이 설명하는 쪽 커밋에 붙이고(`okf/api/products.md` 는
  API 를 바꾼 `[brand]` 커밋에), 어느 쪽인지 애매하면 따로 떼어 `[common]` 커밋으로 만든다.
...
- 커밋하기 전에 계획을 먼저 한 줄씩 보여준다. 태그는 경로로 정해지므로 파일 목록이 곧 근거다 —
  ...
  ```
  1. feat:[brand] 상품 응답에 할인율 필드 추가 — brand/…/ProductService.java, okf/api/products.md
  2. feat:[brand-fo] 상품 상세에 할인율 표시 — brand-fo/…/ProductDetail
  3. chore:[common] 에이전트 스킬 추가 — .claude/skills/**, skills-lock.json
  ```
...
`<태그>` 는 `brand`, `brand-fo`, `brand-bo`, `common` 중 하나다.
...
- `<type>` 과 `[` 사이에 공백이 없고, `]` 다음에 공백 하나가 온다 — `feat:[brand] 추가`.
